...
int service::run_event_service(HCKey hcKey, boost::string_view query_params) {
    Event e(query_params.to_string());
    switch (hcKey) {
        case HCKey::logout :
            std::make_shared<EventRepository>(*db_manager_)->create_event(e);
            break;

        case HCKey::login :
            std::make_shared<EventRepository>(*db_manager_)->update_event_data(e);
            break;

        case HCKey::profile :
            std::make_shared<EventRepository>(*db_manager_)->delete_event(e._id);
            break;
