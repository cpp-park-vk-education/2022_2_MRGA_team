...
Connector::HttpRequestConnector(const string &host, const string &port)
    : host(host), port(port), resolver(ioc), stream(ioc) {}

// Метод, возвращающий текстовый запрос beast::http::request
str_request Connector::string_request(const string &target,
...
void Connector::test_request() {
    ...
    std::cout << res.result_int() << " " << res.result() << " " << res.reason()
              << std::endl;
...
void Connector::set_host(const string &new_host) { host = new_host; }

// задать новый порт
void Connector::set_port(const string &new_port) { port = new_port; }

// при изменении host или порт нужно вызвать этот метод для подключения к
// серверу.
void Connector::resolve_url() { url = resolver.resolve(host, port); }
...
const string &Connector::get_host() const { return host; }
// получение порта подключения
const string &Connector::get_port() const { return port; }
